...
public class Movement : MonoBehaviour 
{
	...
	{		
		while(true)
		{
			//Vector2 start = new Vector3(myTransform.position.x - 0.5f, myTransform.position.y - spriteRenderer.bounds.extents.y - 0.005f);
			Vector2 start = new Vector3(myTransform.position.x - 0.75f, myTransform.position.y - 0.75f);

			Debug.DrawLine(myTransform.position - Vector3.up * 0.75f,  myTransform.position - Vector3.up * 1f, Color.red);
			if(Physics2D.Raycast(start, -Vector2.up, 0.25f))
			{
				grounded = true;	
			}
			else
